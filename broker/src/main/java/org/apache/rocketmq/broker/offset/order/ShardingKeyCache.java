...
public class ShardingKeyCache {
     ...
     * 可用消息信息
     */
    public static class AvailableMessage {
        ...
        public AvailableMessage(String topic, String group, int queueId, String shardingKey, GetMessageResult messageResult) {
            this.topic = topic;
            this.group = group;
        ...
        public GetMessageResult getMessageResult() { return messageResult; }
    ...
    private final ConcurrentHashMap<String, ConcurrentLinkedQueue<AvailableMessage>> availableMessagesMap;
    ...
    public void addAvailableMessage(String topic, String group, int queueId, String shardingKey, 
        ...
        ConcurrentLinkedQueue<AvailableMessage> queue = availableMessagesMap.computeIfAbsent(
                queueKey, k -> new ConcurrentLinkedQueue<>());
        
        // 检查队列大小限制
        if (queue.size() >= MAX_QUEUE_SIZE) {
            AvailableMessage removed = queue.poll();
            if (removed != null) {
        ...
        AvailableMessage availableMessage = new AvailableMessage(topic, group, queueId, shardingKey, messageResult);
     ...
     * @return 可用消息列表
     */
    public List<AvailableMessage> getAvailableMessages(String topic, String group, int queueId, int maxCount) {
        String queueKey = MessageShardingKeyUtil.buildTopicGroupQueueIdentifier(topic, group, queueId);
        ConcurrentLinkedQueue<AvailableMessage> queue = availableMessagesMap.get(queueKey);
        ...
        List<AvailableMessage> result = new ArrayList<>();
        int count = 0;
        
        while (count < maxCount && !queue.isEmpty()) {
            AvailableMessage message = queue.poll();
            if (message == null) {
                break;
            }
     ...
     * @return 可用消息列表
     */
    public List<AvailableMessage> getAvailableMessagesByShardingKey(String topic, String group, int queueId, 
        ...
        List<AvailableMessage> result = new ArrayList<>();
        List<AvailableMessage> toRequeue = new ArrayList<>();
        int count = 0;
        ...
        // 将不匹配的消息重新放回队列
        for (AvailableMessage message : toRequeue) {
            queue.offer(message);
        }
    ...
    public void clearQueueCache(String topic, String group, int queueId) {
        String queueKey = MessageShardingKeyUtil.buildTopicGroupQueueIdentifier(topic, group, queueId);
        ConcurrentLinkedQueue<AvailableMessage> queue = availableMessagesMap.remove(queueKey);
    ...
    public void cleanupExpiredMessages() {
        ...
        for (ConcurrentLinkedQueue<AvailableMessage> queue : availableMessagesMap.values()) {
            if (queue.isEmpty()) {
                continue;
            }
    ...
    public static class CacheStatistics {
        ...
        public long getTotalCachedMessages() { return totalCachedMessages; }
